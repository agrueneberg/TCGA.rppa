    ...
    TCGA.loadScript({
     ...
     // A simple store service to bridge between controllers.
        ...
        app.controller("template", function ($scope, $templateCache) {
            ...
            $templateCache.put("main.html", '<div ng-controller="main"><div class="navbar affix-top" data-spy="affix" data-offset-top="200"><div class="navbar-inner"><ul class="nav"><li><a href="#samples">Samples</a></li><li><a href="#antibodies">Antibodies</a></li><li><a href="#summary">Summary</a></li><li><a href="#correlations">Correlations</a></li><li><a href="#clusters">Clusters</a></li><li><a href="#export">Export</a></li></ul></div></div><h2><a name="samples">Samples</a></h2><div class="btn-group"><button ng-click="selectAll(samples)" class="btn">Select all</button><button ng-click="selectNone(samples)" class="btn">Select none</button></div><ul><li ng-repeat="sample in samples"><input type="checkbox" ng-model="sample.selected" />&nbsp;<a href="{{sample.uri}}" target="_blank">{{sample.id}}</a></li></ul><h2><a name="antibodies">Antibodies</a></h2><div class="btn-group"><button ng-click="selectAll(antibodies)" class="btn">Select all</button><button ng-click="selectNone(antibodies)" class="btn">Select none</button></div><ul><li ng-repeat="antibody in antibodies"><input type="checkbox" ng-model="antibody.selected" />&nbsp;{{antibody.name}}</li></ul><h2><a name="summary">Summary</a></h2><table class="table table-striped"><thead><tr><th>Antibody</th><th>Median</th><th>Mean</th><th>Standard deviation</th></tr></thead><tbody><tr ng-repeat="item in summary"><td>{{item.antibody}}</td><td>{{item.median}}</td><td>{{item.mean}}</td><td>{{item.standardDeviation}}</td></tr></tbody></table><h2><a name="correlations">Correlation coeffcients of antibody pairs</h2><heatmap data="correlations" /><h2><a name="clusters">Clustering of correlation coefficients</a></h2><dendrogram labels="clusterLabels" data="clusters" /><h2><a name="export">Export tidied data (for use in R, MATLAB, Google Refine, ...)</a></h2><p>Format: <code>Sample REF</code> \\t <code>Composite Element REF</code> \\t <code>Protein</code> \\t <code>Protein Expression</code></p><a href="{{blobUri}}" download="rppa.tsv" class="btn">Download tidied data</a><div>');
